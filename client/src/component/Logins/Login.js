...
function LoginPage() {
  ...
  // 0304 수정 작성 : 상호형
  const checkTapValues = [{ label: "개인", tabname: "personalCheckbox" }, { label: "기업", tabname: "businessCheckbox" }, { label: "단체", tabname: "organizationCheckbox" }]
              ...
              <div className='tab-wrap'>
                <ul className='tab-item-wrap'>
                  {checkTapValues.map((key, index) => (
                    <li key={key.label} id={key.tabname} value={index + 1} className={tabItemClass(key.tabname)} onClick={handleOnClick}>
                      {key.label}
                    </li>
                  ))}
                </ul>
